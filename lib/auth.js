...
async function rateLimit(req, bucket, ident, limit, windowSeconds) {
  ...
  try {
    ...
    // Falha ao falar com o KV (fora do ar, DNS, etc.) não pode derrubar
    // login/register/etc inteiro — rate limiting é defesa em profundidade,
    // não a única linha de defesa (senha ainda passa por bcrypt.compare,
    // JWT continua exigido). Deixa passar e loga pra investigar.
    console.error('rateLimit: KV indisponível, deixando passar:', e.message);
    return true;
  }
}
...
// Verifica o JWT e resolve pro registro do usuário no Postgres. Usado por
// todo endpoint de CRUD que precisa do id numérico (chave estrangeira nas
// tabelas), não só do username que vem no token.
async function requireUser(req) {
  const auth = verify(req);
  if (!auth) return null;
  const { getUserByUsername } = require('./users');
  return getUserByUsername(auth.u);
}

module.exports = { kv, sign, verify, readBody, norm, rateLimit, timingSafeCompare, httpError, IS_PROD, requireUser };
