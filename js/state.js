...
function gd(m, y) {
  ...
  return r ? JSON.parse(r) : { salary: 0, income: [], expenses: [], bills: [] };
}
function sd(d, m, y) {
  if (DEMO) return demoRO();
  localStorage.setItem(mk(m, y), JSON.stringify(d));
  syncPush();
}
function gg() {
  return JSON.parse(rawGet('mvf3_goals') || '[]');
}
function sg(g) {
  if (DEMO) return demoRO();
...
function gi() {
  return JSON.parse(rawGet('mvf3_invest') || '[]');
}
function si(i) {
  if (DEMO) return demoRO();
...
function gb() {
  return JSON.parse(rawGet('mvf3_budgets') || '{}');
}
function sb(b) {
  if (DEMO) return demoRO();
