...
module.exports = async (req, res) => {
  ...
  if (String(password).length < 6 || String(password).length > MAX_PASSWORD) return res.status(400).json({ error: `Senha precisa ter entre 6 e ${MAX_PASSWORD} caracteres.` });
  if (String(question).trim().length > MAX_QA || String(answer).trim().length > MAX_QA) return res.status(400).json({ error: 'Pergunta ou resposta muito longa.' });
