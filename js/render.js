...
function renderNet(data) {
  const el = document.getElementById('netStrip');
  if (!el) return;
  const inc = data.income.reduce((a, b) => a + parseFloat(b.value), 0);
  const exp = data.expenses.reduce((a, b) => a + parseFloat(b.value), 0);
  const bpd = data.bills.filter((b) => b.paid).reduce((a, b) => a + parseFloat(b.value), 0);
...
function renderInc(data) {
  ...
  if (!data.income.length) {
    el.innerHTML = '<div class="emp">Nenhuma receita.<br>Defina sua renda mensal acima ou clique em + Adicionar.</div>';
    return;
  }
  el.innerHTML = data.income
    .map(
      (i) =>
        `<div class="li"><div class="lidot" style="background:${CL[i.cat] || '#6b6b72'}"></div><div class="lii"><div class="lin">${esc(i.name)}${i.recur ? '<span class="recpill">mensal</span>' : ''}</div><div class="lic">${esc(i.cat)}</div></div><div class="lia g">${brl(i.value)}</div>${rowActs('income', i.id, !i._salary)}</div>`,
    )
    .join('');
}
...
function renderExp(data) {
  ...
  if (!data.expenses.length) {
    el.innerHTML = '<div class="emp">Nenhuma despesa.<br>Clique em + Adicionar para lançar um gasto.</div>';
    return;
  }
  el.innerHTML = data.expenses
    .map(
      (e) =>
        `<div class="li"><div class="lidot" style="background:${CL[e.cat] || '#6b6b72'}"></div><div class="lii"><div class="lin">${esc(e.name)}${e.recur ? '<span class="recpill">mensal</span>' : ''}</div><div class="lic">${esc(e.cat)}</div></div><div class="lia r">${brl(e.value)}</div>${rowActs('expense', e.id, true)}</div>`,
    )
    .join('');
}
...
function dcl(ds) {
  const d = Math.round((new Date(ds + 'T12:00:00') - new Date()) / 86400000);
  return d < 0 ? 'var(--re)' : d <= 5 ? 'var(--am)' : 'var(--gr)';
}

function renderBills(data) {
  const el = document.getElementById('billsList');
  if (!data.bills.length) {
    el.innerHTML = '<div class="emp">Nenhuma conta cadastrada.<br>Clique em + Nova conta para começar.</div>';
  } else {
...
function renderGoals() {
  ...
  const goals = gg();
  if (!goals.length) {
    el.innerHTML = '<div class="emp">Nenhuma meta criada.<br>Defina um objetivo de economia com + Nova meta.</div>';
    return;
  }
  el.innerHTML = goals
    .map((g) => {
...
function renderInvest() {
  ...
  if (!invs.length) {
    el.innerHTML = '<div class="emp">Nenhum ativo cadastrado.<br>Adicione seus investimentos com + Adicionar ativo.</div>';
    if (es) es.innerHTML = '';
    return;
  }
...
function renderHist(data) {
  ...
  if (!all.length) {
    el.innerHTML = '<div class="emp">Nenhuma movimentação encontrada</div>';
  ...
  let tin = 0,
    tout = 0;
  all.forEach((t) => {
    if (t.kind === 'income') tin += parseFloat(t.value);
    else tout += parseFloat(t.value);
  });
  el.innerHTML = all
    .map(
    ...
    .join('');
  if (foot)
    foot.innerHTML = `<span>${all.length} item${all.length !== 1 ? 's' : ''}</span><span>Entradas ${brl(tin)} · Saídas ${brl(tout)}</span>`;
}
...
function searchHist(v) {
  ...
  renderHist(data);
  setTimeout(() => {
    const s = document.getElementById('histSrch');
    if (s) {
      s.focus();
...
function renderBudget(data) {
  ...
  [...data.expenses, ...data.bills.filter((b) => b.paid)].forEach((e) => {
    spent[e.cat] = (spent[e.cat] || 0) + parseFloat(e.value);
  });
  let totB = 0,
    totS = 0;
  el.innerHTML = EC.map((c) => {
    const lim = parseFloat(budgets[c] || 0),
      sp = spent[c] || 0;
    totB += lim;
    totS += sp;
    ...
    const tag =
      lim <= 0
        ? '<span class="budget-note">sem limite</span>'
        : ratio > 1
          ? `<span class="tag ov">${brl(sp - lim)} acima</span>`
          : ratio >= 0.8
            ? `<span class="tag sn">${pct}%</span>`
